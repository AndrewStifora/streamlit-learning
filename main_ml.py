...
def get_fvalue(val):
    func_feature_dict = {"No": 1, "Yes": 2}
    for key, value in func_feature_dict.items():
        if val == key:
            return value
...
if app_mode == 'Home':
    ...
    st.image('media/slider-short-3.jpg')
    ...
    if st.button("Predict"):
        with open("media/6m-rain.gif", "rb") as file_:
            contents = file_.read()
        ...
        if prediction[0] == 0:
            ...
            st.markdown(f'<img src="data:image/gif;base64,{data_url_no}" alt="">',
